// Command swahili_zulu trains a learned sound-correspondence model on the
// Swahili → Zulu Bantu cognate corpus in cognates.tsv and prints a
// human-readable report. The corpus exercises noun-class prefix alternations,
// spirantization before high front vowels, and Proto-Bantu consonant shifts.
//
// Run with:
//
//	cd experiments/swahili_zulu && go run .
package main
...
// graphemes used in the Swahili/Zulu corpus.
func parseSegments(ipa string) []regulae.Segment {
	multi := []string{"tʃ", "dʒ", "tʰ", "pʰ", "kʰ", "aː", "eː", "iː", "oː", "uː"}
	runes := []rune(ipa)
...
func loadCorpus(path string) ([]labeledPair, error) {
	...
	hasBreaks := len(header) >= 5 && header[3] == "swahili_breaks" && header[4] == "zulu_breaks"
	...
	for sc.Scan() {
		...
		gloss, sw, zu := parts[0], parts[1], parts[2]
		var swBreaks, zuBreaks []int
		if hasBreaks && len(parts) >= 5 {
			swBreaks = parseBreaks(parts[3])
			zuBreaks = parseBreaks(parts[4])
		}
		out = append(out, labeledPair{
			gloss: gloss,
			src: regulae.Form{
				LectID:         "swahili",
				Segments:       parseSegments(sw),
				MorphemeBreaks: swBreaks,
			},
			tgt: regulae.Form{
				LectID:         "zulu",
				Segments:       parseSegments(zu),
				MorphemeBreaks: zuBreaks,
			},
		})
	}
	return out, sc.Err()
...
func main() {
	...
	fmt.Printf("Loaded %d Swahili → Zulu cognate pairs.\n", len(labeled))

	cognateSets := regulae.CognateSetsFromPairs(pairs, [2]string{"swahili", "zulu"}, "swahili_zulu")
	multiModel, err := regulae.TrainModel(cognateSets, regulae.DefaultTrainOptions())
	if err != nil {
		fmt.Fprintln(os.Stderr, "train error:", err)
		os.Exit(1)
	}
	trained, ok := multiModel.PairwiseModel("swahili", "zulu")
	if !ok {
