// Command tone_chinese_like trains a learned model on a synthetic
// Mandarin/Cantonese-like corpus (noisy tonal correspondences) and prints
// a report. This experiment is designed to expose cross-dimensional
// discovery failure modes on data where tonal mappings are irregular.
//
...
// Run with:
//
//	cd experiments/tone_chinese_like && go run .
package main
...
// parseTonedForm splits an IPA string like "san1" into segments, attaching
...
func main() {
	...
	fmt.Printf("Loaded %d Mandarin/Cantonese-like toned pairs.\n", len(labeled))
	fmt.Println()
	fmt.Println("NOTE: synthetic fixture. Designed to expose cross-dimensional")
	fmt.Println("(tonogenesis) failures in purely tone-to-tone tonal machinery.")
	fmt.Println()
