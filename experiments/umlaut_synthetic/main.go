// Command umlaut_synthetic trains a multi-lect model on 40 synthetic CVCV
// pairs embodying an umlaut rule (a → æ when the next syllable's vowel is
// front) and prints the recovered correspondences together with any long-range
// conditioned entries. This is a report experiment, not a pass/fail test.
//
// Run with:
//
...
func main() {
	...
	fmt.Printf("Loaded %d synthetic umlaut pairs.\n", len(labeled))
	fmt.Println()
	fmt.Println("Rule: a → æ when next syllable's vowel is front (i, e).")
	fmt.Println("      Context discovery cannot capture this (the intervening C")
	fmt.Println("      doesn't encode the triggering V2 feature).")
	fmt.Println()
