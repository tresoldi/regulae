// Command latin_french trains a learned sound-correspondence model on the
// Latin→French cognate corpus in cognates.tsv and prints a human-readable
// report. French shows heavy nasalization, loss of final consonants, and
// aggressive palatalization — a more extreme reduction profile than Spanish.
//
// Run with:
//
//	cd experiments/latin_french && go run .
package main
...
var multi = []string{"tʃ", "dʒ", "nj", "ɛ̃", "ɑ̃", "ɔ̃", "œ̃"}
...
func loadCorpus(path string) ([]labeledPair, error) {
	...
	hasBreaks := len(header) >= 5 && header[3] == "latin_breaks" && header[4] == "french_breaks"
	...
	for sc.Scan() {
		...
		tgt := regulae.Form{LectID: "french", Segments: parseSegments(parts[2])}
...
func showAlignments(labeled []labeledPair, model *regulae.LearnedModel) {
	toShow := []string{
		"father", "night", "eight", "hundred", "head",
		"tongue", "name", "five", "wolf", "king", "water",
	}
...
func findContextProblems(model *regulae.LearnedModel) {
	...
	fmt.Println("(context discovery should have split most real cases; remaining competitions")
	fmt.Println(" are either spurious or below BIC threshold)")
	fmt.Println(strings.Repeat("=", 70))
	if len(competing) == 0 {
...
func main() {
	...
	fmt.Printf("Loaded %d Latin → French cognate pairs.\n\n", len(labeled))
	fmt.Println("Training learned model...")

	cs := regulae.CognateSetsFromPairs(pairs, [2]string{"latin", "french"}, "lf")
	...
	trained, ok := multiModel.PairwiseModel("latin", "french")
	if !ok {
