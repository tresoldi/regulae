...
rg_status rg_context_is_segment(const rg_context *ctx, const char *grapheme, bool *out) {
    ...
    *out = 0;
    ...
    if (ctx->cache->features[slot].grapheme == 0) {
        ...
        if (ctx->cache->features[slot].grapheme == 0) {
            /* Not a failure of the question that was asked. merkmal already
             * answered it and the answer is in *out; all that was lost is the
             * memo, and the next ask will pay for it again.
             *
             * The accessors that return a borrowed pointer into the cache --
             * rg_context_features_internal, rg_context_displacement_internal --
             * cannot do this and return RG_ERR_OOM instead, because what they
             * hand back *is* the cached object and there is nothing to point
             * at. The difference between the two is forced by what they
             * return, not a disagreement about policy. */
            return RG_OK;
        }
...
/* Resolves a grapheme in the feature cache and hands back its entry, asking
 * merkmal only on the first ask.
 *
 * Both the feature set and the derived context constraints are memoised on the
 * one entry, and both callers used to probe for it separately -- constraints
 * called features_internal, which found the entry, and then walked the same
...
rg_status rg_context_constraints_internal(
    ...
    *out_count = 0;
    ...
    if (entry->constraints != 0) {
        *out = entry->constraints;
        *out_count = entry->constraint_count;
        return RG_OK;
    }
    {
