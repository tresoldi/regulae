 ...
 * `source` holds the grapheme, `count` its running total. The denominator a gap
...
rg_status add_present_count(
    ...
    rg_segment_count_row *next;
    for (i = 0; i < *count; i++) {
        if (strcmp((*rows)[i].source, grapheme) == 0) {
            (*rows)[i].count += weight;
            return RG_OK;
        }
    }
    ...
    (*rows)[*count].source = rg_strdup_internal(grapheme);
    if ((*rows)[*count].source == 0) {
        return RG_ERR_OOM;
    }
    (*rows)[*count].count = weight;
    (*count)++;
