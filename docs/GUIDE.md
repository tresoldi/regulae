...
## Morpheme boundaries and syllable breaks in wide format
...
third, and `ˈpe-ter` segments to a stressed syllable and a break, with no `-`
reaching the feature system as a sound. A `<lect>_breaks` column, if present,
takes precedence over inline marks so the two are never double-counted.

Morpheme conditioning and stress conditioning are both implemented in the
engine; the wide loader now accepts the marks that let a corpus state them
without switching to the long format.
...
corpus that cannot load. It is generated by running the corpora, so it cannot
drift from what the tool actually does.

The distinction matters. A corpus that cannot load says something about the
